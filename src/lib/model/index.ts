  ...
  try {
    ...
    const response = await client.messages.create({
      ...
      system: [
        ...
        "A fraud warning, dollar amount, routine delivery update, or ordinary deadline is not a scam signal by itself.",
        "Genuine alerts often ask for a short keyword reply such as YES, NO, STOP, HELP, or one letter, or say to call the number on the back of a card. Do not treat those actions as suspicious by themselves.",
        "Judge only actions the message explicitly requests. A warning not to share a code is protective, and mentioning details or next steps elsewhere is not an implicit request to share credentials or tap a link. If no risky action is requested, choose none even when the message reports something alarming.",
        "Choose amber when pressure pushes the recipient toward a risky action: use a link to verify, pay, sign in, claim money, or fix an unexpected problem; share a one-time code, password, or card number; call an unfamiliar number supplied in the message to fix a problem; or send money or gift cards.",
        "Do not flag a link merely because it appears with urgency. Its explicitly stated purpose must match a risky action above; a link to information, a survey, or a routine service task does not match by itself.",
